...
高斯--赛德尔迭代法是把线性方程组改写成迭代格式，然后从一个初始值出发，不断计算新的近似解，直到结果满足要求为止。若记其迭代矩阵为 $B_{GS}=-(D+L)^{-1}U$，则当 $\rho(B_{GS})<1$ 时迭代收敛；本题中 $\rho(B_{GS})\approx 0.996901<1$，因此该方法收敛，但由于谱半径非常接近 $1$，所以收敛速度较慢。
